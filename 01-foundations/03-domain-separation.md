...
Gossip-rs maintains a **domain registry** in `crates/gossip-contracts/src/identity/domain.rs` with 18 domain constants.
...
| `GIT_MIRROR_PATH_V1` | `gossip/git/v1/mirror-path` | Derive-key | Stable mirror-cache path derivation for repo-native Git scans |
...
**Property**: If you add a 19th domain but forget to update the array length, compilation fails:
