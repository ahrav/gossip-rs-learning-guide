...
The `Arc` wrapper allows the engine to be shared between the runtime and the driver without cloning the engine's internal data structures (compiled regex automata, anchor lookup tables, transform configurations). The `ScanDriver::run` method takes `Arc<scanner_engine::Engine>`, so the driver holds a reference-counted handle. When the driver drops (after `run` returns), the reference count decrements. The engine is freed only when the last reference is dropped -- which, for the cached default engine, is at process exit.
