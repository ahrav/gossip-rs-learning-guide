...
The `Arc` wrapper allows the engine to be shared between the runtime and the scan loop without cloning the engine's internal data structures (compiled regex automata, anchor lookup tables, transform configurations). The source-family modules (`ordered_content`, `git_repo`) receive `Arc<scanner_engine::Engine>`, so the scan holds a reference-counted handle. When the scan completes, the reference count decrements. The engine is freed only when the last reference is dropped -- which, for the cached default engine, is at process exit.
...
[Chapter 3](03-event-and-commit-sinks.md) examines the four event output formats (JSONL, Text, JSON, SARIF), the `CoordinationEventSink` for distributed event recording, and the `DurableCommitSink` that derives the full identity chain from engine-level finding records.
